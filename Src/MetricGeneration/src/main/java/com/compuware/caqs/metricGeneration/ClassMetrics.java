...
public class ClassMetrics extends Check {
    ...
    public void leaveClassDef(DetailAST aAST) {
        String type = "";
        String visu = "";
        int nl = aAST.findFirstToken(TokenTypes.OBJBLOCK).findFirstToken(TokenTypes.RCURLY).getLineNo() -
                aAST.getLineNo();
        ...
        if (Utils.isPublic(aAST)) {
            ...
            if (aAST.findFirstToken(TokenTypes.MODIFIERS).findFirstToken(TokenTypes.LITERAL_PRIVATE) !=
                    null) {
                visu = "Private";
            } else {
                visu = "Protected";
        ...
        log(aAST, "<elt type=\"CLS\" name=\"" + className + "\" startLine=\"" +
                this.startLine + "\" subtype=\"" + type + "\" visibility=\"" +
                visu + "\">\n" +
                "<parent name=\"" + pkgName + "\" />\n" +
    ...
    public DetailAST setClassLevel(DetailAST c) {
        ...
        while (c.getType() != TokenTypes.CLASS_DEF 
                && c.getType() != TokenTypes.INTERFACE_DEF
                && c.getType() != TokenTypes.ENUM_DEF) {
            c = c.getParent();
    ...
    public String getClassName(DetailAST d) {
        ...
        if ((mycurrentClass.getParent()) != null &&
                mycurrentClass.getParent().getType() != 0) {
        ...
        while ((mycurrentClass = mycurrentClass.getPreviousSibling()) != null) {
            while (mycurrentClass.getParent() != null ||
                    mycurrentClass.getPreviousSibling() != null) {
                if (mycurrentClass.getParent() != null) {
                    mycurrentClass = mycurrentClass.getParent();
                ...
                if (mycurrentClass.getType() == TokenTypes.CLASS_DEF) {
                    ...
                    className = mycurrentClass.findFirstToken(TokenTypes.IDENT).getText() +
                            "#" + className;
                }
    ...
    public String getPackageName(DetailAST d) {
        ...
        while (tmpCurrentClass.getPreviousSibling() != null ||
                tmpCurrentClass.getParent() != null) {
            if (tmpCurrentClass.getParent() != null) {
                tmpCurrentClass = tmpCurrentClass.getParent();
            }
            if (tmpCurrentClass.getPreviousSibling() != null) {
                tmpCurrentClass = tmpCurrentClass.getPreviousSibling();
            }
