package com.compuware.caqs.security.net;
...
public final class AddressMacInfo {
	...
	public final static String getMacAddress() {
		...
		try {
			...
			if (os.startsWith("Windows")) {
				result = windowsParseMacAddress(windowsRunIpConfigCommand());
			}
			else if (os.startsWith("Linux")) {
				result = linuxParseMacAddress(linuxRunIfConfigCommand());
			}
			else if (os.startsWith("Mac OS X")) {
				result = osxParseMacAddress(osxRunIfConfigCommand());
			}
		}
	...
	private final static String windowsParseMacAddress(String ipConfigResponse)
		...
		try {
			...
			logger.error("Error getting Windows MAC address", ex);
	...
	private final static String linuxParseMacAddress(String ipConfigResponse)
		...
		try {
			...
			logger.error("Error getting Linux MAC address", ex);
			throw new ParseException(ex.getMessage(), 0);
		}
	...
	private final static String osxParseMacAddress(String ipConfigResponse)
		...
		try {
			...
			logger.error("Error getting OSX MAC address", ex);
			throw new ParseException(ex.getMessage(), 0);
		}
