		...
		if(new File(dir).isDirectory()){
			...
			for(int i =0; i<fileNames.length;i++){
				...
				for(int j=0;j<numSub;j++){
					...
					results+=fileNames[i].substring(0,fileNames[i].lastIndexOf("."))+"."+subNames[j]+";"+loc+";"+vg+";"+cloc;
	...
	private void subdivideFile(){
		...
		for(int i =0; i < this.contents.length;i++){
			...
			if(beginFunc.matcher(contents[i]).matches()){
				...
				int rparent=0;
				int nbLParent=0;
				while((lparent=subNames[numSub].indexOf("(",lparent+1))>0){
					nbLParent++;      
					llparent=subNames[numSub].indexOf("(",lparent+1);               	
					rparent=subNames[numSub].indexOf(")",lparent+1);
					if(llparent>-1) {
						nbLParent++;
						lparent=llparent;                		
					}           
				...
				if(nbLParent==1) {
					subNames[numSub]=subNames[numSub].substring(0,subNames[numSub].indexOf(")",lparent)+1);
				}else{
					subNames[numSub]=subNames[numSub].substring(0,subNames[numSub].indexOf(")",rparent+1)+1);
				}
				int j=0;
				l = new ArrayList();
