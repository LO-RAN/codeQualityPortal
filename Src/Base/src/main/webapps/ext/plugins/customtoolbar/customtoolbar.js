...
//		Layout all the items.
        ...
        for(var i = 0, len = items.length; i < len; i++) {
            totalWidth = this.renderItem(items[i], i, target, totalWidth);
        }
    },

    // private
	getNextCell : function(c, target, newLine){
        var thistr = (((c.align == 'right') || (this.container.align == 'right') || (this.align == 'right')) ? this.rightTr : this.leftTr);
        if(newLine) {
            var parentNode = thistr.parentNode;
            thistr = parentNode.appendChild(document.createElement('tr'));
            if(((c.align == 'right') || (this.container.align == 'right') || (this.align == 'right'))) {
                this.rightTr = thistr;
            } else {
                this.leftTr = thistr;
            }
        }
		return thistr.appendChild(document.createElement('td'));
    },

    // private
    renderItem : function(c, position, target, totalWidth){
        //totalWidth += c.getEl().getSize().width;
        if(c) {
        	if (c.isFill) { // A Fill just switches sides
	        	...
	        	if (c instanceof Ext.Element) {
                    ...
                    var newLine = false;
                    totalWidth += (c.width)?c.width:5;
                    if((totalWidth+20) > target.getSize().width) {
                        newLine = true;
                        totalWidth = c.width;
                    }
                    ...
                    if (c.rendered){
                        if (this.renderOnLayout) {
			        		...
			        		this.getNextCell(c, target, newLine).appendChild(c[c.wrap ? 'wrap' : 'el'].dom);
			        	}
		        	} else {
			            c.render(this.getNextCell(c, target, newLine));
			        }
			    }
			}
        }
