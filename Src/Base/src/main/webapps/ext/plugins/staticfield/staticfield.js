...
Ext.extend(Ext.ux.form.StaticTextField, Ext.BoxComponent,  {
    ...
    onRender : function(ct, position){
        ...
        if(this.parentClassName!=null) {
            ct.dom.parentNode.className = this.parentClassName;
        }
        
        ct.dom.parentNode.setAttribute("background-color", "transparent");
		
        if(this.smallLine) {
            this.el.removeClass('x-form-text');
            this.el.addClass('ux-form-'+this.parentClassName);
        } else {
    ...
    setValue : function(v){
        ...
        if(this.rendered){
            var inner = v;
            if(this.htmlEncode) {
                inner = Ext.util.Format.htmlEncode(v);
            }
