...
<pre>
   ...
   */
    contextMenu: [{
        name: 'select',
        text: getI18nResource("caqs.dashboard.timeplot.contextMenu.select"),
        iconCls: 'image_resize_actual',
        handler: function() {
            this.setSelectionAction('select');
        }
    },{
        name: 'zoom',
        text: getI18nResource("caqs.dashboard.timeplot.contextMenu.zoom"),
        iconCls: 'magnifier_zoom',
        handler: function() {
            this.setSelectionAction('zoom');
        }
    },{
        name: 'move',
        text: getI18nResource("caqs.dashboard.timeplot.contextMenu.move"),
        iconCls: 'magnifier__arrow',
        handler: function() {
            this.setSelectionAction('move');
        }
    },'-',{
        name: 'actual',
        text: getI18nResource("caqs.dashboard.timeplot.contextMenu.actual"),
        ...
        name: 'selectall',
        text: getI18nResource("caqs.dashboard.timeplot.contextMenu.selectall"),
        handler: function() {
            this.selectAll();
        }
    },{
        name: 'unselectall',
        text: getI18nResource("caqs.dashboard.timeplot.contextMenu.unselectall"),
        handler: function() {
            this.unselectAll();
        }
    },'-',{
        name: 'property',
        text: getI18nResource("caqs.dashboard.timeplot.contextMenu.property"),
        iconCls: 'puzzle',
        handler: function() {
            this.showProperty();
        }
   ...
   */
   datapointContextMenu: [{
        ...
        text: getI18nResource("caqs.dashboard.timeplot.contextMenu.selectalldatapoint"),
        iconCls: 'flag_plus',
        handler: function() {
            var item = this.clickItem || this.hoverItem;
            if (item) {
                this.selectAll(item.series);
            }
        }
    },{
        name: 'unselectalldatapoint',
        text: getI18nResource("caqs.dashboard.timeplot.contextMenu.unselectalldatapoint"),
        iconCls: 'flag_minus',
        handler: function() {
            var item = this.clickItem || this.hoverItem;
            if (item) {
                this.unselectAll(item.series);
            }
        }
    },'-',{
        name: 'showseries',
        text: getI18nResource("caqs.dashboard.timeplot.contextMenu.showseries"),
        iconCls: 'database_plus',
        handler: function() {
            var item = this.clickItem || this.hoverItem;
            if (item) {
                this.setHidden(item.series, false);
            }
        }
    },{
        name: 'hideseries',
        text: getI18nResource("caqs.dashboard.timeplot.contextMenu.hideseries"),
        iconCls: 'database_minus',
        handler: function() {
            var item = this.clickItem || this.hoverItem;
            if (item) {
                this.setHidden(item.series, true);
            }
        }
    }
    ],
...
<div id="{tipId}">{label} ({0}, {1})</div>
   ...
   */
    ...
    options:            undefined,
    defaultSeriesOptions:undefined,
    flotLoadedSeries:   undefined,
   ...
   */
    ...
    initComponent: function() {
        ...
        Ext.applyIf(this.options, {
                xaxis:      this.retrieveAxe(this.xaxis),
                yaxis:      this.retrieveAxe(this.yaxis),
                legend:     this.legend,
                y2axis:     this.retrieveAxe(this.y2axis),
                x2axis:     this.retrieveAxe(this.x2axis),
   ...
   */
    ...
    onRender: function(ct, position) {
        ...
        var series = this.series;
        if (this.data) {
            series = this.setupData(this.data, series);
        }
        if (series) {
    ...
    afterRender: function() {
        ...
        $('#' + this.id).bind('plotunselected', function(event, range) {
            flot.onPlotUnselected(event, range);
        });
        ...
        if (this.tooltip) {
            this.on(this.tooltipEvent, function(flot, event, pos, item) {
                if (item) {
                    this.showTooltip(event, pos, item, false);
                }
            }, this);
        }
    },
    ...
    setupLegend: function() {
        ...
        if(!legend_tbody) {
            if(this.options && this.options.legend && this.options.legend.containerId) {
                legend_tbody = Ext.get(this.options.legend.containerId).query("table > tbody")[0]
            }
        }
        if (legend_tbody) {
            var series = this.getData();
            ...
            for (var i = 0, len = series.length; i < len; i++) {
                ...
                if (idx >= 0) {
                    ...
                    cb.on('click', this.onLegendClick, this, {
                        series: s
                    });
                    lb.on('click', this.onLegendClick, this, {
                        series: s
                    });
    ...
    // private
    onPlotUnselected: function(event, ranges) {
        return this.fireEvent('plotunselected', this, event, ranges);
    },

    // private
    onPlotSelected: function(event, ranges) {
        if (this.fireEvent('plotselected', this, event, ranges) !== false) {
            if (!this.mouseDownEvent || !this.mouseUpEvent) {
                return;
            }
            var diff_px = [
            ...
            switch (this.selection.action) {
                case 'select':
                    if (!this.lastEvent[this.selection.appendKey]) {
                        this.unselectAll();
                    }
                    ...
                    for (var i = 0, len = d.length; i < len; i++) {
                        for (var j = 0, dlen = d[i].datapoints.length; j < dlen; j++) {
                            if (this.addSelected(d[i].series, d[i].datapoints[j])) {
                                changed = true;
                            }
                        }
                    }
...
/**
   ...
   * @param {Object} ranges zooming range
   */
zoom: function(ranges) {
    Ext.apply(this.options.xaxis, { min: ranges.xaxis.from, max: ranges.xaxis.to });
    this.createPlotObject();
},

resetPlotZoom: function() {
    this.zoom(this.plotBaseRanges);
},

/**
   * Zoom by ratio for current dispaly range
...
zoomRatio: function(ratio) {
    ...
    if(b != null) {
        ...
        if (b.xaxis)  {
            r.xaxis  = {
                min: b.xaxis.min  * ratio,
                max: b.xaxis.max  * ratio
                };
    }
    if (b.yaxis)  {
        r.yaxis  = {
            min: b.yaxis.min  * ratio,
            max: b.yaxis.max  * ratio
            };
}
if (b.x2axis) {
    r.x2axis = {
        min: b.x2axis.min * ratio,
        max: b.x2axis.max * ratio
        };
}
if (b.y2axis) {
    r.y2axis = {
        min: b.y2axis.min * ratio,
        max: b.y2axis.max * ratio
        };
}
this.zoom(r);
}
...
syncSelected: function() {
    for (var i = 0; i < this.selected.length; i++) {
        ...
        if(selected!=null && selected.series !=null) {
...
addSelected: function(series, datapoint) {
    if (!this.selection || !series.selectable) {
        return false;
    }
...
selectAll: function(_series) {
    ...
    if (typeof _series == 'string') {
        _series = this.findSeries(_series);
    }
    var changed = false;
    var series = this.getData();
    this.selected = [];
    for (var i = 0, len = series.length; i < len; i++) {
        if (typeof _series != 'undefined' && _series != series[i]) {
            continue;
        }
        if (!series[i].selectable) {
            continue;
        }
        var selected = {
            series: series[i],
...
unselectAll: function(_series) {
    ...
    for (var i = 0, len = this.selected.length; i < len; i++) {
        var s = this.selected[i];
        if (typeof _series != 'undefined' && _series != s.series) {
            continue;
        }
...
setActionDisabled: function(key, disabled) {
    if (this.actions[key]) {
        this.actions[key].setDisabled(disabled);
    }
},
...
showTooltip: function(event, pos, item, forceUpdate) {
    ...
    var o = {
        ...
        1:           dp[1],
        //begin edit
        caqsCustom1: item.series.data[item.dataIndex][3],//nombre d'analyses pour le jour donne
        caqsCustom2: item.series.data[item.dataIndex][4],
        caqsCustom3: item.series.data[item.dataIndex][5],
        caqsCustom4: item.series.data[item.dataIndex][6],
        caqsCustom5: item.series.data[item.dataIndex][7],
        //end edit
...
clearTooltip: function(tooltipCmp) {
    ...
    if (tooltipCmp == this.tooltipCmp) {
        this.tooltipCmp = null;
    }
    tooltipCmp.destroy();
},
...
findSeries: function(name) {
    var series = (this.flot) ? this.flot.getData() : this.flotLoadedSeries;
    for (var i = 0, len = series.length; i < len; i++) {
        var idx = series[i].dataIndex || series[i].label;
        if (name === idx || name === series[i].id) {
            return series[i];
        }
    }
    return null;
...
setupSeries: function(series) {
    if (typeof series.length == 'undefined') {
        series = [series];
    }
    for (var i = 0, len = series.length; i < len; i++) {
        var s = series[i];
...
setSeries: function(series) {
        this.loadedSeries = series;
        this.plot();
        this.store.loadData(this.createStoreData());
        this.plotBaseRanges = {
            xaxis: {
                from: this.flot.getAxes().xaxis.datamin,
                to: this.flot.getAxes().xaxis.datamax
            }
        }
    },
/**
...
applySeriesAndOptions: function(s, i) {
    var retour = {};
    if(s instanceof Array) {
        if(s[i] instanceof Array) {
            ...
            Ext.applyIf(retour, s[i]);
        }
    } else {
        if(s.datas && s.datas) {
            if(s.datas instanceof Array) {
                Ext.applyIf(retour, {
                   data: s[i].datas
                });
            } else {
                Ext.applyIf(retour, s.datas[i]);
            }
        }
    }
    if(this.seriesOptions && this.seriesOptions[i]) {
        Ext.applyIf(retour, this.seriesOptions[i]);
    }
    if(this.defaultSeriesOptions) {
        Ext.applyIf(retour, this.defaultSeriesOptions);
    }
    return retour;
},
...
plot: function() {
    ...
    if(this.loadedSeries != null) {
        if(this.loadedSeries instanceof Array) {
            for (var i = 0, len = this.loadedSeries.length; i < len; i++) {
                _series[i] = this.applySeriesAndOptions(this.loadedSeries, i);
            }
        } else {
            if(this.loadedSeries.xaxisTicks) {
                this.xaxis.ticks = this.loadedSeries.xaxisTicks;
                Ext.apply(this.options.xaxis, this.xaxis );
            }
            if(this.loadedSeries.datas) {
                if(this.loadedSeries.datas instanceof Array) {
                    for (var i = 0, len = this.loadedSeries.datas.length; i < len; i++) {
                        _series[i] = this.applySeriesAndOptions(this.loadedSeries.datas, i);
                    }
                }
            }
        }
    ...
    if (this.fireEvent('beforedraw', this, _series) !== false) {
        this.flotLoadedSeries = _series;
        this.createPlotObject(_series);
        this.setupLegend();
    }
},
...
createPlotObject: function(_series) {
    if(_series==null) {
        _series = this.flotLoadedSeries;
    }
    this.flot = $.plot($('#' + this.id),
            _series, this.options);

},


    retrieveAxe: function(axe) {
        var retour = axe;
        if(typeof axe == 'string') {
            if(axe == 'true') {
                retour = {min: 0};
            } else if(axe == 'caqsAnalysisScore') {
                retour = {min: 0, max: 4.5,
                                color: 'rgba(0,0,0,1.0)',
                                tickFormatter: function (v, axis) {return v.toFixed(axis.tickDecimals);}
                            };
            } else if( axe == 'caqsTimeAxe') {
                retour = {mode: "time"};
            }
        }
        return retour;
...
setHidden: function(series, hidden) {
    ...
    if(series instanceof Array) {
        ...
        for(var i=0; i<l; i++) {
            var serie = series[i];
            var s = this.findSeries(serie.label);
            s.hidden = hidden;
        }
        redraw = true;
    } else {
...
for (var i = 0; i < series.length; ++i)
   ...
   */
getData: function() {
    return (this.flot) ? this.flot.getData() : this.flotLoadedSeries;
},
/**
