...
Ext.ux.ManagedIFrame = function(){
    ...
    el.updateManager=new Ext.UpdateManager(el,true)
    el.updateManager.showLoadIndicator= config.showLoadIndicator || false;
        ...
        switch(rstatus){
            ...
            case 'domready': //MIF
              ...
              if(this._domFired = this._hooked = this._renderHook() ){
                 this.fireEvent("domready",this);  //Only raise if sandBox injection succeeded (same domain)
              }
            case 'domfail': //MIF
              this._domReady = true;
              this.hideMask();
 ...
 Ext.ux.ManagedIframePanel = Ext.extend(Ext.Panel, {
    ...
    */
    ...
    initComponent : function(){
        ...
        if(this.unsupportedText){
            unsup ={html:Ext.DomHelper.markup(this.unsupportedText)};
        }
    ...
    onRender : function(ct, position){
        ...
        if(this.iframe = this.body.child('iframe.x-managed-iframe')){
            ...
            this.iframe = new Ext.ux.ManagedIFrame(this.iframe, {loadMask:this.loadMask,showLoadIndicator:this.showLoadIndicator});
    ...
    */
    setSrc : function(url, discardUrl,callback){
         ...
         if(discardUrl !== true){ this.defaultSrc = {url:src,callback:callback}; } //nomalize
         this.saveState();
         return this;
    },
