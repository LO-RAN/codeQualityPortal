...
public class EvolutionXmlGenerator extends XmlGenerator {

    Collection newAndBadElements;
    Collection oldAndWorstElements;
    Collection oldAndBetterElements;
    Collection oldBetterWorstElements;
    Collection stableElements;
    Collection badAndStableElements;
     ...
     * @param loc
     */
    public EvolutionXmlGenerator(ElementBean eltBean, double noteSeuil, Locale loc) {
    ...
    public void retrieveData() {
        ...
        newAndBadElements = evolution.retrieveNewAndBadElements(eltBean.getId(), eltBean.getBaseline().getId(), filter);
        oldAndWorstElements = evolution.retrieveOldAndWorstElements(eltBean.getId(), eltBean.getBaseline().getId(), filter);
        oldAndBetterElements = evolution.retrieveOldAndBetterElements(eltBean.getId(), eltBean.getBaseline().getId(), filter);
        oldBetterWorstElements = evolution.retrieveOldBetterAndWorstElements(eltBean.getId(), eltBean.getBaseline().getId(), filter);
        stableElements = evolution.retrieveStableElements(eltBean.getId(), eltBean.getBaseline().getId(), filter);
        badAndStableElements = evolution.retrieveBadAndStableElements(eltBean.getId(), eltBean.getBaseline().getId(), filter);
    ...
    public void generate(File destination) throws IOException {
        PrintWriter out = createWriter(destination, ReportConstants.EVOLUTION_XML_FILE_NAME
    ...
    private boolean evolutionDataIsEmpty() {
        return oldAndWorstElements.isEmpty() && oldAndBetterElements.isEmpty()
                && oldBetterWorstElements.isEmpty() && stableElements.isEmpty();
    }

    private void generateEvolution(PrintWriter out) {
        addGroup("NEWBAD", this.newAndBadElements, out);
        addGroup("OLDWORST", this.oldAndWorstElements, out);
        addGroup("OLDBETTER", this.oldAndBetterElements, out);
        addGroup("OLDBETTERWORST", this.oldBetterWorstElements, out);
        addGroup("STABLE", this.stableElements, out);
        addGroup("BADSTABLE", this.badAndStableElements, out);
    ...
    private void generateGoalEvolution(PrintWriter out) {
        ...
        for (FactorBean fb : this.factorEvolution) {
            ...
            out.print("<GOAL lib=\"" + lib + "\" note=\"" + this.nf.format(fb.getNote()
                    - 0.005) + "\" tendance=\"" + tendanceLabel + "\" />");
        }
    ...
    private void generateEvolution(ElementEvolutionSummaryBean evolutionBean, PrintWriter out) {
        ...
        out.print(" desc=\"" + getXmlCompliantString(evolutionBean.getDesc())
                + "\"");
        out.print(" nbcrit=\""
                + this.intf.format(evolutionBean.getNbCriterions()) + "\"");
        out.println("/>");
    }
}
