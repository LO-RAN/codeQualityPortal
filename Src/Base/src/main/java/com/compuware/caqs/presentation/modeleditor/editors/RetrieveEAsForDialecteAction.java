 ...
 * @author cwfr-dzysman
 */
public class RetrieveEAsForDialecteAction extends ExtJSGridAjaxAction {
    ...
    protected JSONObject retrieveDatas(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        String idDialecte = request.getParameter("idDialecte");
        Map<String, List<ElementBean>> result = null;
        if (idDialecte != null) {
            result = ElementSvc.getInstance().retrieveAllApplicationEntitiesForDialecte(idDialecte);
        } else {
