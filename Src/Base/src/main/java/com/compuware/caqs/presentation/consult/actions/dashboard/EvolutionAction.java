...
public class EvolutionAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping,
        ...
        if(elt != null) {
        	...
        	File evolutionFile = evolutionSvc.retrieveDashboardEvolutionImage(elt, imgConfig);

        	if (evolutionFile != null && evolutionFile.exists()) {
    	        try {
    	        	ServletOutputStream out = response.getOutputStream();
    	            byte[] result = ImageUtil.getInstance().getImageBytes(evolutionFile);
    	            out.write(result);
    	            out.flush();
    	        }
    	        catch (IOException e) {
    	            logger.error("Error reading file: "+evolutionFile.getName(), e);
    	        }
        	}
        }
