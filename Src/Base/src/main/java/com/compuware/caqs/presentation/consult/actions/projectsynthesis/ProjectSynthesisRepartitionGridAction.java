 ...
 * @author cwfr-dzysman
 */
public class ProjectSynthesisRepartitionGridAction extends ExtJSGridAjaxAction {
    ...
    public JSONObject retrieveDatas(ActionMapping mapping, ActionForm form,
        ...
        if (elts != null) {
            object = this.retrieveFactorRepartition(elts, request);
        }
     ...
     * @param projects
     */
    private JSONObject retrieveFactorRepartition(List<ElementBean> eas,
        ...
        for (ElementBean ea : eas) {
            factors.addAll(synthese.retrieveKiviat(ea, ea.getBaseline().getId()));
        }
