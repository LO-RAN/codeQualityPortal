...
public class ProjectActionPlanPrintAction extends Action {
    ...
    /**
     * Cree un tableau donnant la liste des elements impactes
     * @param doc Le document
     * @param request La requete
     * @param elts La liste des elements impactes
     * @param docWriter Le PdfWriter
    ...
    private void addElementListTable(
        ...
        for (ActionPlanImpactedElementBean elt : elts) {
            ...
            for (int j = 0; j < taille; j++) {
                criterionsList += "- " + elt.getCriterions().get(j).getInternationalizableProperties().getLib(loc);
                if (j < (taille - 1)) {
                    criterionsList += "\n";
                }
            }
...
/*            ActionPlanUnit actionPlanUnit = ActionPlanSvc.getInstance().getActionPlanUnitForModel(ea.getUsage().getId());

            int uoNbUnit = actionPlanUnit.getNbUnits();
            String uoUnit = actionPlanUnit.getLib(loc);

            double totalWorkUnitCost = uoNbUnit * totalCost;

            String msg = resources.getString("caqs.actionplan.totalCost");
            msg = MessageFormat.format(msg, new Object[]{uoNbUnit + " " + uoUnit});
            p.add(new Chunk(msg, this.getTextFont()));

            String calcul = nf.format(totalCost) + " * " + uoNbUnit + " " +
                    uoUnit + " = " + nf.format(totalWorkUnitCost) + " " + uoUnit +
                    ".";
            p.add(new Chunk(calcul, this.getBoldTextFont()));
            doc.add(p);*/

            this.addTermCriterionSection(doc, request, criterions, ActionPlanPriority.SHORT_TERM, totalCost, docWriter, loc, resources);
            this.addTermCriterionSection(doc, request, criterions, ActionPlanPriority.MEDIUM_TERM, totalCost, docWriter, loc, resources);
            this.addTermCriterionSection(doc, request, criterions, ActionPlanPriority.LONG_TERM, totalCost, docWriter, loc, resources);
            this.addOtherCommentsSection(doc, request, ap.getElements(), docWriter, loc, resources);
        }
    }
    ...
    private void addOtherCommentsSection(
        ...
        if (!criterions.isEmpty()) {
            ...
            for (ActionPlanCriterionBean criterion : criterions) {
                ...
                Paragraph p = new Paragraph();
                p.add(new Chunk(criterion.getInternationalizableProperties().getLib(loc) + " : ", this.getUnderlinedTextFont()));
                doc.add(p);
    ...
    private void createCriterionsSection(
        ...
        if (criterions != null && !criterions.isEmpty()) {
            ...
            for (ActionPlanCriterionBean criterion : criterions) {
                ...
                this.addMultiLinedTextToParagraph(" : " + criterion.getInternationalizableProperties().getDesc(loc), p, doc);
                doc.add(p);
