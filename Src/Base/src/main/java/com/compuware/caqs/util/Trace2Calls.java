...
public class Trace2Calls {
	...
	/**
	 * process : requirements are that the input file be a clean Uniface profiling trace that has been sorted by sequence Id.
	 * i.e. typically, the input file should have been produced by CleanTrace.java
	 * <pre>
	 * Produced files will be those expected by CAQS as input for loading using the ""McCabe" loader format : 
	 * - ALLCLASSESMETRICS.CSV 			: list of components and libraries;
	 * - traitedALLMODULESMETRICS.CSV 	: list of operations, procedures and triggers;
	 * - CALLSTO.MODIFIED.CSV			: list of couples "caller;callee" at operation/procedure/trigger level
	 * 
	 * It will also produce, as a goodie :
	 * - sequence.sdx					: sequence file in a format compatible with SDEdit (cf. http://sdedit.sourceforge.net)
	 * </pre>
	 * @param inputFile the clean and ordered profiling trace to process 
	 * @param outputDir where to put the produced output files
	 *
	 * @see CleanTrace
	 */
	public void process(String inputFile, String outputDir) {
		try {
			...
			// format actual sdx file that can be parsed by SDEdit
