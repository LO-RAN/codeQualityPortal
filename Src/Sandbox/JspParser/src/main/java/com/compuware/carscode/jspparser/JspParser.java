 ...
 */
 ...
 /*@bgen(jjtree) DoctypeExternalId */
  ...
  public int countLine(List nodeList) {
	  int result = 0;
	  if (nodeList != null) {
		  SimpleNode n;
		  ...
		  while (i.hasNext()) {
			  n = (SimpleNode)i.next();
			  result += n.getEndLine() - n.getBeginLine() + 1;
		  }
	  }
	  return result;
  }

  public void analyse() throws JaxenException {
	  ASTCompilationUnit result = CompilationUnit();
	  System.out.println("NB Lines JSP Comments: " + countLine(result.findChildrenOfType(ASTJspComment.class)));
	  System.out.println("NB Lines HTML Comments: " + countLine(result.findChildrenOfType(ASTCommentTag.class)));
	  System.out.println("NB Lines JSP Scriptlet: " + countLine(result.findChildrenOfType(ASTJspScriptlet.class)));
	  System.out.println("NB JSP Expression: " + countLine(result.findChildrenOfType(ASTJspExpression.class)));
	  System.out.println("NB JSP Expression in attribute: " + countLine(result.findChildrenOfType(ASTJspExpressionInAttribute.class)));
	  System.out.println("NB Lines Javascript: " + countLine(result.findChildNodesWithXPath("//Element[@Name='script']/Content")));
	  System.out.println("NB Lines Global: " + result.getEndLine());
  }
  
  public static void main(String[] args) throws IOException, JaxenException {
	  JspCharStream stream = new JspCharStream(new FileReader("D:\\CarsCode\\Referentiel\\Src\\Base\\src\\main\\webapps\\synthese.jsp")); 
	  JspParser parser = new JspParser(stream);
	  parser.analyse();
  }
