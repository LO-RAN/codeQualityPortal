 ...
 *
 */
public class PhpcsConnector extends AbstractConnector {
   ...
   private static final String OS_SPECIFIC_DOUBLE_QUOTE = OS_NAME.compareToIgnoreCase("WINDOWS") >= 0 ? "\"" : "";
	...
	/**
	 * @param options The script options to set.
	 */
	public void setScriptOptions(String s) {
		this.scriptOptions = s;
	}
    ...
    private String getCommand() {
    	...
    	/*if (this.includePath != null && this.includePath.length() > 0) {
    		cmd.append(" -d include_path=\"").append(this.includePath).append("\"");
    	}*/
    	if (this.scriptPath != null && this.scriptPath.length() > 0) {
    		cmd.append(" -f ").append(this.scriptPath);
    	}
    	if (this.scriptOptions != null && this.scriptOptions.length() > 0) {
    		cmd.append(" -- ").append(this.scriptOptions);
    	}
    	cmd.append(" --output="+this.outputFile+"");
