...
public class EntityImporter {
    ...
    public void processEntity(ImportEntityDef entityDef) throws SQLException, IOException {
        ...
        DelimiterParser delimiterParser = entityDef.getDelimiterParser();
        ...
        try {
            ...
            if (entityDef.getSourceEncoding() != null) {
                reader = new BufferedReader(new InputStreamReader(new FileInputStream(entityDef.getSource()), entityDef.getSourceEncoding()));
            } else {
                reader = new FileReader(entityDef.getSource());
            }

            if (entityDef.getNumBytesToSkip() > 0) {
                reader.skip(entityDef.getNumBytesToSkip());
            }
            delimiterParser.setReader(reader);
            ColumnValue[] values = null;
            String nextRow = delimiterParser.getNextRow();
            while (nextRow != null) {
                List beforeImportQueries = entityDef.getBeforeImportQueries();
                if (beforeImportQueries != null
                        && !beforeImportQueries.isEmpty()) {
                    try {
                        values = delimiterParser.getValues(nextRow);
                        if (trimValues) {
                            for (int i = 0; i < values.length; i++) {
                                if (values[i].isString() && values[i].getString()
                                        != null) {
                                    values[i].setString(values[i].getString().trim());
                                }
                            }
                        }
                        for (Iterator it = beforeImportQueries.iterator(); it.hasNext();) {
                            BeforeImportEntity bie = (BeforeImportEntity) it.next();
                            PreparedStatement pstmt = null;
                            try {
                                pstmt = connection.prepareStatement(bie.getQuery());
                                List bieParams = bie.getParameters();
                                if (bieParams != null) {
                                    int i = 1;
                                    for (Iterator it2 = bieParams.iterator(); it2.hasNext();) {
                                        Integer bieParam = (Integer) it2.next();
                                        if (bieParam != null) {
                                            pstmt.setString(i, values[bieParam.intValue()].getString());
                                            i++;
                                        }
                                    }
                                }
            ...
            if (reader != null) {
                try {
                    ...
                    LOG.warn("Could not close reader for file '"
                            + entityDef.getSource() + "'", ie);
                }
            }
        ...
        for (int i = 0; i < columns.length; i++) {
            ...
            if (def.getType() == ColumnDef.UNKNOWN_SQL_TYPE) {
                ...
                if (!columnToTypes.containsKey(def.getName().toLowerCase())) {
                    throw new SQLException("Column '" + def.getName()
                            + "' not found!");
                }
                def.setType(((Short) columnToTypes.get(def.getName().toLowerCase())).intValue());
                LOG.debug("Found SQL Type '" + def.getType() + "' for column '"
                        + def.getName() + "'");
            } else {
                LOG.debug("Predefined SQL Type '" + def.getType()
                        + "' for column '" + def.getName() + "'");
            }
            ...
            if (formatPattern == null) {
                ...
                LOG.debug("Using format '" + formatPattern + "' for column '"
                        + def.getName() + "'");
            ...
            if (nextColumnDef instanceof GenerateColumnDef) {
                ...
                for (int i = 0; i < dependencies.length; i++) {
                    if (dependencies[i].indexOf(".") != -1) {
                        throw new IOException("Invalid dependency '"
                                + dependencies[i] + "' for generated column "
                                + "'" + nextColumnDef.getName()
                                + "', only simple dependencies allowed");
                    }
     ...
     */
    protected boolean importNextRow(EntityImporterState state, String nextRow) {
        ImportEngine curEngine = state.engine;
        ImportEntityDef entityDef = state.entityDef;
        DelimiterParser delimiterParser = state.delimiterParser;
        ...
        try {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("Parsed Column Values = ["
                        + columnValuesToString(values) + "]");
            }
            ...
            if (rowTranslator != null) {
                ...
                if (LOG.isDebugEnabled()) {
                    if (values == null) {
                        ...
                        LOG.debug("Row Translated Column Values = ["
                                + columnValuesToString(values) + "]");
                    }
                }
                ...
                if (values == null) {
                    ...
                    fireImportEntityRowEvent(new ImportEntityRowEvent(this, entityDef, nextRow, origValues, ImportEntityRowEvent.STATE_ERROR, null));
                    return false;
                }
            }
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("Filled Column Values = ["
                        + columnValuesToString(values) + "]");
            }

            curEngine.importRow(values);
            if (LOG.isDebugEnabled()) {
                LOG.debug("Imported Column Values = ["
                        + columnValuesToString(values) + "]");
            }

            if (batchCount > 1 && supportsBatch) {
                batchRows.add(nextRow);
                fireImportEntityRowEvent(new ImportEntityRowEvent(this, entityDef, nextRow, values, ImportEntityRowEvent.STATE_BATCH, null));
            } else {
                fireImportEntityRowEvent(new ImportEntityRowEvent(this, entityDef, nextRow, values, ImportEntityRowEvent.STATE_SUCCESS, null));
            }
            return true;
        } catch (SQLException s) {
            LOG.debug("SQLException caught while trying to import row", s);
            fireImportEntityRowEvent(new ImportEntityRowEvent(this, entityDef, nextRow, values, ImportEntityRowEvent.STATE_ERROR, s));
            return false;
        } catch (MalformedDataException male) {
            LOG.debug("MalformedDataException caught while trying to import row", male);
            male.setRow(nextRow);
            fireImportEntityRowEvent(new ImportEntityRowEvent(this, entityDef, nextRow, values, ImportEntityRowEvent.STATE_ERROR, male));
            return false;
        }
    }
    ...
    protected ColumnValue[] fillInValues(EntityImporterState state, ColumnValue[] values) throws MalformedDataException {
        ImportEntityDef entityDef = state.entityDef;
        LOG.trace("fillInValues( <ImportEntityDef:" + entityDef.getTable()
                + ">, <ColumnValue[]:" + values.length + "> ) ->");
            ...
            for (int i = 0; i < values.length; i++) {
                ...
                if (values[i].isString()) {
                    ...
                    buf.append("<InputStream:"
                            + values[i].getInputStreamLength() + ">");
                }
            }
            throw new MalformedDataException("# of values not equal to # of columns "
                    + "(" + (values.length + state.numGeneratedColumns) + ","
                    + columns.length + ") " + buf.toString());
        }
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            if (values[i] == null || "".equals(values[i].getString())) {
                if (columnDef.getDefaultValue() != null) {
                    LOG.debug("Setting default value '"
                            + columnDef.getDefaultValue() + "' for column '"
                            + columnDef.getName() + "'");
            ...
            if (translator != null) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Value before translator '" + values[i]
                            + "' for column '" + columnDef.getName() + "'");
                }

                values[i] = translator.getValue(columnDef, values[i]);

                if (LOG.isDebugEnabled()) {
                    LOG.debug("Value after translator '" + values[i]
                            + "' for column '" + columnDef.getName() + "'");
                }
            }
        }
    ...
    protected int processBatchUpdateException(ImportEntityDef entityDef, List batchRows, BatchUpdateException bue) {
        ...
        for (int i = 0; i < batchRows.size(); i++) {
            if (i < rowStatus.length) {
                if (rowStatus[i] == 1 || rowStatus[i]
                        == ImportEngine.BATCH_SUCCESS_ROWS_UNKNOWN) {
    ...
    private String resolveCatalogName(DatabaseMetaData metaData, String catalog) throws SQLException {
        ...
        if (resolvedCatalog == null) {
            throw new SQLException("Could not find catalog '" + catalog
                    + "' in DatabaseMetaData ");
        }
    ...
    private String resolveSchemaName(DatabaseMetaData metaData, String catalog, String schema) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            if (((nextCatalog == null && catalog == null)
                    || nextCatalog.equals(catalog))
        ...
        if (resolvedSchema == null) {
            throw new SQLException("Could not find schema '" + schema
                    + "' of catalog '" + catalog + "' in DatabaseMetaData ");
        }
    ...
    private String resolveTableName(DatabaseMetaData metaData, String catalog, String schema, String table) throws SQLException {
        ...
        if (resolvedTable == null) {
            throw new SQLException("Could not find table '" + table + "'"
                    + " of schema '" + schema + "' of catalog '" + catalog
                    + "' in DatabaseMetaData ");
        }
