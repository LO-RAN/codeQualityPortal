...
public class ImportEntityDefElement extends EntityDefElement {
    ...
    public ImportEntityDef getEntityDef() throws BuildException {
        ...
        if (delimiterElement == null) {
            throw new BuildException("No element 'delimiter' found in entity '"
                    + entityDef.getTable() + "'");
        }
        ...
        if (this.beforeImportQueries != null
                && !this.beforeImportQueries.isEmpty()
                && (entityDef instanceof ImportEntityDef)) {
            for (int i = 0; i < this.beforeImportQueries.size(); i++) {
                BeforeImportEntity bie = (BeforeImportEntity) this.beforeImportQueries.get(i);
                ((ImportEntityDef)entityDef).addBeforeImportQuery(bie);
            }
        }
        ...
        try {
            ...
            throw new BuildException("ImportEngine Class Not Found : '"
                    + engineClassname + "'");
        } catch (IllegalAccessException e) {
            throw new BuildException("ImportEngine Class Not Instantiated : '"
                    + engineClassname + "'");
    ...
    /**
     * queries to execute before import
     */
    List beforeImportQueries;

    public BeforeImportEntity createBeforeImport() {
        if (this.beforeImportQueries == null) {
            this.beforeImportQueries = new ArrayList();
        }
        BeforeImportEntity q = new BeforeImportEntity();
        this.beforeImportQueries.add(q);
        return q;
    }

    public List getBeforeImportQueries() {
        return this.beforeImportQueries;
    }
}
