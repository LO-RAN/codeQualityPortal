...
public class XMLDelimiterParser implements DelimiterParser 
{
	...
	public String getNextRow() throws IOException 
	{
		if ( xslResultReader == null )
		{
			...
			while ( result != -1 ) 
			{
				...
				if  ( buf.length() == rowsElementName.length()+2 )
				{
					if ( !("<"+rowsElementName+">").equals( buf.toString() ) )
					{
						buf.delete(0,1);
					}
					else
			...
			// only if there was at least one row to process
			if ( count>0 && !("<"+rowsElementName+">").equals( buf.toString() ) )
			{
				try
				{
		...
		{
			buf.append( nextChar[0] );
			if ( buf.length() > rowElementName.length()+3 && ("</"+rowElementName+">").equals( buf.substring(buf.length()-(rowElementName.length()+3),buf.length()) ) )
			{
				break;
			}
			else if ( buf.length() >= rowsElementName.length()+3 && ("</"+rowsElementName+">").equals( buf.substring(buf.length()-(rowsElementName.length()+3),buf.length()) ) )
			{
				buf.delete(buf.length()-(rowsElementName.length()+3),buf.length());
				break;
			}
			result = xslResultReader.read(nextChar);
		}
		...
		throws MalformedDataException 
	{
		...
		if ( !"r".equals( parent.getNodeName() ) )
		{
			throw new MalformedDataException( "Root of document is not named 'r', but '"+parent.getNodeName()+"'", nextRow, null );
		}
		...
		for ( int i = 0; i < children.getLength(); i++ )
		{
			...
			if ( !"c".equals( children.item(i).getNodeName() ) )
			{
				throw new MalformedDataException( "Expected node named 'c' for next column value,"+
