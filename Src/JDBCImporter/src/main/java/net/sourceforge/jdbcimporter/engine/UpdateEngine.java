 ...
 * determines whether to use an insert or update sql statement by executing a
 * select (based on the primary keys of the table). It will fail if the data to
 * be imported does not contain a column that is part of the primary key.
 * 
 * @version 0.6
 * @author Chris Nagy
 */
public class UpdateEngine implements ImportEngine {
	...
	/**
	 * The property on an entity definition whose value will be appended to the
	 * select statement.
	 */
	public static final String SELECT_POSTFIX_PROPERTY = "selectPostfix";

	/**
	 * The property on a column definition that will indicate the the column is
	 * a primary key (the value should be 'true').
	 */
	public static final String PRIMARY_KEY_PROPERTY = "primaryKey";

	/**
	 * The property on a column definition whose value will be used as the
	 * update expression for that column (the value should contain the character
	 * '?').
	 */
	...
	/**
	 * The select prepared statement used to figure out whether the insert or
	...
	public void init() throws SQLException {
		String fullTableName = helper.getFullyQualifiedTableName(connection
				.getMetaData(), entityDef.getCatalog(), entityDef.getSchema(),
				entityDef.getTable());
		...
		// if there is at least one column beyond primary keys
		if (columns.length > uniqueKeys.length) {
			...
			for (int i = 0; i < columns.length; i++) {
				...
				if (i < columns.length - uniqueKeys.length) {
					...
					if (columnDef.getProperty(UPDATE_EXPRESSION_PROPERTY) != null) {
						String expression = columnDef
								.getProperty(UPDATE_EXPRESSION_PROPERTY);
						if (expression.indexOf("?") != -1) // TODO: More error
						// checking
						{
	 ...
	 */
	public void importRow(ColumnValue[] values) throws SQLException,
		...
		if (insertFlag) {
			stmt = insertStmt;
			if (batchFlag)
				stmtTypes.add("insert");
		} else {
			stmt = updateStmt;
			if (batchFlag)
				stmtTypes.add("update");
		}
	...
	protected List getDefinedPrimaryKeys() {
		...
		for (int i = 0; i < columns.length; i++) {
			if ("true".equalsIgnoreCase(columns[i]
					.getProperty(PRIMARY_KEY_PROPERTY))) {
	 ...
	 * @throws SQLException
	 */
	protected List getPrimaryKeys(String catalog, String schema, String table)
			throws SQLException {
		LOG.trace("getPrimaryKeys( '" + catalog + "', '" + schema + "', '"
				+ table + "' )");
		List primaryKeyNames = new ArrayList();

		DatabaseMetaData dbMeta = connection.getMetaData();
		ResultSet rset = dbMeta.getPrimaryKeys(catalog, schema, table);

		while (rset.next()) {
	           // test whether we already have this column in our set or not
	           // only add if not already present.
	           // This should not happen but has been encountered with 
	           // a connection to Oracle on Linux for one particular table :
	           // key column names where contained twice in the result set !
	           if ( ! primaryKeyNames.contains(rset.getString("COLUMN_NAME")))
	           {
	...
	public void executeBatch() throws BatchUpdateException {
		...
		try {
			...
			try {
				...
				LOG
						.error(
								"SQLException while trying to execute batch insert statements",
								sqle);
				if (uniqueKeys != null) {
					...
					insertRowStates = helper.resolveRowStates(connection,
							entityDef, insertRows, uniqueKeys, new int[0]);
				} else {
			...
			for (int i = 0; i < rowsUpdated.length; i++) {
				if ("insert".equals(stmtTypes.get(i))
						&& insertCounter < insertRowStates.length) {
			...
			try {
				...
				LOG
						.error(
								"SQLException while trying to execute batch update statements",
								sqle);
				...
				int[] selectKeys = new int[updateColumnMap.length
						- uniqueKeys.length];
				System.arraycopy(updateColumnMap, 0, selectKeys, 0,
						selectKeys.length);
				updateRowStates = helper.resolveRowStates(connection,
						entityDef, updateRows, uniqueKeys, selectKeys);
			}

			int updateCounter = 0;
			for (int i = 0; i < rowsUpdated.length; i++) {
				if ("update".equals(stmtTypes.get(i))
						&& updateCounter < updateRowStates.length) {
			...
			if (updateStmt != null) {
				...
				for (int i = 0; i < rowsUpdated.length; i++) {
					if (rowsUpdated[i] != 1
							&& rowsUpdated[i] != BATCH_SUCCESS_ROWS_UNKNOWN) {
						BatchUpdateException bue;
						if (updateEx != null && insertEx != null) {
							MultipleBatchUpdateException mBue = new MultipleBatchUpdateException(
									rowsUpdated);
							mBue.setNextExceptions(new SQLException[] {
									insertEx, updateEx });
							bue = mBue;
