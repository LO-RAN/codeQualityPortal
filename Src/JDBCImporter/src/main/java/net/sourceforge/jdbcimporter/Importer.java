...
import net.sourceforge.jdbcimporter.config.*;
...
/**
 * The Importer class is the main class that controls the import
...
public class Importer
{
	...
	public Importer()
	{
		...
		{
			Map columnGeneratorMap = CustomObjectMapping.getDefaultColumnValueGeneratorMapping();
	 ...
	 * 
	 * @param input import config as xml
	 * @throws IOException if the import config cannot be read
	 */
	 ...
	 * 
	 * @param pluginProperties the defined plugin mappings
	 */
	public void initPlugins( Properties pluginProperties )
	{
		...
		while ( keys.hasMoreElements() )
		{
			...
			else if ( nextKey.startsWith( DELIMITER_PREFIX ) )
			{
				...
				LOG.debug("Adding DelimiterParser mapping '"+delimiterType+"', '"+delimiterClass+"'" );
			...
			{
				String rowTranslatorType = nextKey.substring(ROWTRANSLATOR_PREFIX.length());
				String rowTranslatorClass = pluginProperties.getProperty( nextKey );
				LOG.debug("Adding RowTranslator mapping '"+rowTranslatorType+"' = '"+rowTranslatorClass+"'");
				rowTranslatorFactory.addMapping(rowTranslatorType,rowTranslatorClass);
			}
			else
			{
	 ...
	 * 
	 * @param engineClassname the classname of the import engine
	 */	
	 ...
	 * 
	 * @throws FileNotFoundException if the log file or bad log file is not valid
	 * @throws IOException if the log file or bad log file cannot be opened and written to
	 */
	public void beginImport() throws FileNotFoundException, IOException
	{
		...
		try
		{
			engine = (ImportEngine) Class.forName( engineClassname ).newInstance();	
		}
		...
		if ( LOG.isDebugEnabled() ) 
			LOG.debug( "Initializing Connection "+importDef.getConnectionDef() );
		...
		{
			LOG.warn("Could not set auto commit to '"+(importDef.getCommitCount() <= 1)+"'", e );
		}
		...
		if( importDef.getBatchCount() > 1 && !autoCommit )
		{
			...
			{
				LOG.warn("Unable to detect if connection has batch mode support", s );
			}
		...
		{
			importer.addImportListener( (ImportListener) this.importListeners.get(i) );
		}
		if ( importDef.getPreSQLFile() != null )
		{
			executeSQLFile( engine.getConnection(), importDef.getPreSQLFile() );
		}
		fireImportEvent( new ImportEvent( this, importDef, this.engineClassname, supportsBatch ), true );		
		ImportEntityDef[] entities = importDef.getEntities();
		boolean errorOccurred = false;
		for ( int i = 0; i < entities.length; i++ )
		{
			fireImportEntityEvent( new ImportEntityEvent( this, entities[i], null ), true );
			try
			{
				importer.processEntity( entities[i] );
				fireImportEntityEvent( new ImportEntityEvent( this, entities[i], null ), false );
			}
		...
		fireImportEvent( new ImportEvent( this, importDef, this.engineClassname, supportsBatch ), false );
	 ...
	 * 
	 * @param listener an import listener
	 */
	 ...
	 * 
	 * @param e the import event
	 * @param start true if the import is starting and false if the import is finished
	 */
	 ...
	 * 
	 * @param e the import entity event
	 * @param start true if the import of the entity is starting and false if the
	...
	protected void executeSQLFile( Connection connection, File SQLFile )
	{
		...
		{
			LOG.error("IOException while reading file '"+SQLFile.getAbsolutePath()+"'", e  );
		}
		catch ( SQLException e )
		{
			LOG.error("SQLException while executing statment '"+buf.toString()+"'", e );
		}
		finally
		{
			if ( stmt != null )
	...
	public static void main(String[] args)
	{
		...
		if ( args.length < 1 )
		{
			System.out.println("Usage: java net.sourceforge.jdbcimporter.Importer <import config file> [plugin file]");
			System.exit(-1);
		}
		try
		{
			...
			try
			{
				...
				importer.setEngine( System.getProperties().getProperty("jdbcimporter.engine", "net.sourceforge.jdbcimporter.engine.BasicEngine") );
				importer.setFailonerror( "true".equals( System.getProperties().getProperty( "jdbcimporter.failonerror" ) ) );
			...
			finally
			{
				if ( fileInput != null )
				{
					...
					{
						LOG.warn("Could not close file '"+importConfigFile+"'", e );
					}
				}	
			}
