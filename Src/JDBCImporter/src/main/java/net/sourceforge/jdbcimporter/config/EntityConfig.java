...
public class EntityConfig {
     ...
     *
     */
    public static final String ENTITY_BEFORE_IMPORT_TAG = "beforeImport";
    public static final String ENTITY_BEFORE_IMPORT_QUERY_PROPERTY = "query";
    public static final String ENTITY_BEFORE_IMPORT_PARAMS_PROPERTY = "parameters";
    /**
    ...
    public EntityDef getEntity(Node node) throws DOMException, InvalidCustomObjectDefException {
        ...
        if (table == null || "".equals(table.getNodeValue())) {
            throw new DOMException(DOMException.NOT_FOUND_ERR, ImportConfig.ENTITY_TAG
                    + " attribute: " + ENTITY_TABLE_ATTR);
        }
        ...
        while (child != null && ENTITY_PROPERTY_TAG.equals(child.getNodeName())) {
            ...
            if (propertyName != null && !"".equals(propertyName.getNodeValue()) && propertyValue
                    != null && !"".equals(propertyValue.getNodeValue())) {
        ...
        child = ImportConfig.getFirstElementChild(node);
        while (child != null
                && ENTITY_BEFORE_IMPORT_TAG.equals(child.getNodeName())) {
            NamedNodeMap childAttrs = child.getAttributes();
            Node query = childAttrs.getNamedItem(ENTITY_BEFORE_IMPORT_QUERY_PROPERTY);
            Node queryParameters = childAttrs.getNamedItem(ENTITY_BEFORE_IMPORT_PARAMS_PROPERTY);
            if (query != null && !"".equals(query.getNodeValue())) {
                BeforeImportEntity e = new BeforeImportEntity();
                e.setQuery(query.getNodeValue());
                String params = queryParameters.getNodeValue();
                e.setParameters(params);
                if (entityDef instanceof ImportEntityDef) {
                    ((ImportEntityDef) entityDef).addBeforeImportQuery(e);
                }
            }
            child = ImportConfig.getNextElementSibling(child);
        }
    ...
    protected void addColumn(EntityDef entityDef, Node column) throws DOMException, InvalidCustomObjectDefException {
        ...
        if (name == null || "".equals(name.getNodeValue())) {
            throw new DOMException(DOMException.NOT_FOUND_ERR, COLUMN_TAG
                    + " attribute: " + COLUMN_SQLTYPE_ATTR);
        }
        ...
        while (child != null) {
            if (COLUMN_PROPERTY_TAG.equals(child.getNodeName())) {
                ...
                if (propertyName != null
                        && !"".equals(propertyName.getNodeValue()) && propertyValue
                        != null && !"".equals(propertyValue.getNodeValue())) {
